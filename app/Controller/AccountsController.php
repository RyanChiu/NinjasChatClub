...
class AccountsController extends AppController {
	...
	function __sendemail($subject = 'empty', $content = 'empty',
		$from = 'SUPPORT@ninjaschatclub.com',
		$mailto = 'SUPPORT@ninjaschatclub.com',
		$replyto = 'SUPPORT@ninjaschatclub.com') {
		...
		$cakeEmail = new CakeEmail(
			array(
				...
				'username'=>'SUPPORT@ninjaschatclub.com',
				'password'=>'NCC123',
				'transport' => 'Smtp'
			)
		);
	...
	function login() {
		...
		if (!empty($this->request->data)) {//if there are any POST data
			...
			if(preg_match('/[\'\"\|]/',$this->request->data['Account']['username'])){
				...
				file_put_contents('/tmp/ninja_inject.log',print_r($elog,true) . "\n",FILE_APPEND);
				return;
			}
	...
	function forgotpwd() {
		...
		if (!empty($this->request->data)) {
			...
			$r = $this->Account->find('first',
				array(
					'conditions' => array(
						'lower(username)' => strtolower(mysql_escape_string($this->request->data['Forgot']['username']))
					)
				)
			);
			if (empty($r)) {
