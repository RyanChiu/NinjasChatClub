<?php 
/**
 * this driver is for the new site, it'll be abbrivated with "agc1" (keep diff from the others)
 */
...
if (true || $ip == "66.180.199.11" || $ip == "127.0.0.1") {
	...
	if($agent !== mysql_escape_string($agent)) { 
		file_put_contents('./logs/inject.log',"SQL INJECT '$agent from {$_SERVER['REMOTE_ADDR']}'.\n",FILE_APPEND); 
		echo "Invalid Agent"; 
		exit;
	}
	...
	$chsfrombbr = explode(",", AGC1_CHS);;// !!! MUST MAKE SURE ABOUT THIS ARRAY WITH Jesse
	$i = 0;
	$chs_exist = false;
	while ($r = mysql_fetch_assoc($rs)) {
		if ($chsfrombbr[$i] == $ch) {
			...
			if ($type == 'sale') {
				 ...
				 */
				$tsql = sprintf(
					"select * from stats where siteid = %d and typeid = %d and transactionid = %d", 
					$siteid, $typeid, $trxid
				);
			...
			if (!$donothing) {
				...
				error_log(
					"do nothing, cause transactionid '$trxid' of site id '$siteid' and type id '$typeid' already exists.\n",
					3,
					$logpath
				);
			}
